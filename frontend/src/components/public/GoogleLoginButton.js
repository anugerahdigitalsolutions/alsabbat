...
export const GoogleLoginButton = ({ label = 'Masuk dengan Google', testId = 'google-login-button' }) => {
  ...
  const ready = Boolean(config?.google_enabled && config?.google_client_id);
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        disabled={!ready}
        aria-disabled={!ready}
        className="als-focus flex min-h-[44px] w-full items-center justify-center gap-3 rounded-[var(--radius-sm)] border bg-white text-sm font-semibold transition-colors duration-200 hover:bg-[rgba(0,0,0,0.03)] disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:bg-white"
      ...
      {config && !ready ? (
        <p className="text-[11px]" style={{ color: 'var(--muted-fg)' }} data-testid={`${testId}-not-configured`}>
          Login Google belum dikonfigurasi. Gunakan email dan kata sandi.
        </p>
      ) : null}
    </div>
  );
