...
export default function BarayaMemberCardPage() {
  usePageSeo({
    ...
    description: 'Kartu member digital AL SABBAT.',
    path: '/akun/kartu',
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Kartu Member AL SABBAT',
          text: `AL SABBAT ${card?.member_number}`,
          url: verifyUrl,
        });
        return;
      } catch (e) {
  ...
  return (
    <div data-testid="page-member-card">
      <PublicPageHeader
        ...
        description="Kartu resmi member AL SABBAT. QR hanya berisi tautan verifikasi publik, bukan data pribadi."
