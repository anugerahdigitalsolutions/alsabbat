...
export default function MemberVerifyPage() {
  ...
  usePageSeo({
    ...
    description: 'Verifikasi kartu member AL SABBAT.',
  ...
  const state = !result?.found
    ...
    : result.valid
    ? { Icon: BadgeCheck, title: 'Member AL SABBAT Terverifikasi', tone: 'rgba(252,207,43,0.18)', color: 'var(--club-secondary)' }
  ...
  return (
    <div data-testid="page-member-verify">
      <PublicPageHeader
        label="Verifikasi"
        title="Verifikasi Member"
        description="Pemeriksaan resmi kartu member AL SABBAT."
          ...
          <div className="als-card mx-auto max-w-xl p-7 text-center" data-testid="member-verify-result">
            ...
            {result?.found ? (
              ...
              <p className="mt-4 text-sm" style={{ color: 'var(--muted-fg)' }}>
                Kartu ini tidak terdaftar sebagai member AL SABBAT.
              </p>
            )}
